...
/loop 1h "/data-analyst 실행"
```

오른쪽 표에 세 가지를 비교해봤어요.

**첫 번째는 `/loop`예요.**
CLI 세션 안에서 실행되고, 최소 간격은 1분이에요.
개발이나 테스트 중에 모니터링할 때 쓰기 좋아요.
세션이 끊기면 멈춥니다.

**두 번째는 Desktop Tasks예요.**
Claude Code 앱이 실행 중일 때 동작하고, 일상 반복 작업에 쓰기 좋아요.
앱을 종료하면 멈춰요.

**세 번째는 Routines예요.**
Anthropic 클라우드에서 실행돼요.
앱을 꺼도 계속 돌아갑니다.
최소 간격은 1시간이에요.

요약하면 테스트 중이면 `/loop`, 일상 반복이면 Desktop Tasks, 24시간 무인 자동화가 필요하면 Routines입니다.
