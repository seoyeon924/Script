...
에이전트가 5명이에요. Echo가 팀장이고 나머지 4명이 순서대로 일합니다.

**STEP 1** — CSV 파일을 `runs/latest/input/` 폴더에 넣어요. 어떤 CSV든 상관없어요.

**STEP 2** — 텔레그램 Echo 봇에 이렇게 보내면 돼요.

```
runs/latest/input/ 에 CSV 있어. 파이프라인 실행해줘.
```

이게 전부예요. 나머지는 에이전트가 알아서 해요.

**STEP 3** — 5명이 순서대로 실행돼요. Aria가 데이터 프로파일이랑 EDA를 하고, Sam이 문제 정의랑 KPI를 뽑고, Min이 분석 리포트를 쓰고, Evan이 대시보드랑 경영진 보고서를 만들어요. 각자 파일로 저장해요.

**STEP 4** — 브라우저에서 `activity-monitor.html` 열어두면 10초마다 자동 갱신되면서 완료된 스테이지가 ✓로 바뀌어요.

**STEP 5** — 다 끝나면 텔레그램으로 결과 파일 목록이 자동으로 와요.
