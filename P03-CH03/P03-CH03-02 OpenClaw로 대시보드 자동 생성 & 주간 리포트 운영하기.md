...
실제로 어떤 결과물이 나오는지 보여줄게요. 키트에 포함된 샘플 데이터는 광고 성과 데이터예요. 구글·유튜브·메타 등 채널별로 노출수, 클릭수, 광고비, 전환수가 담겨있어요.

Evan이 이 데이터로 만든 대시보드를 보면 — 채널별 전환 성과를 한눈에 볼 수 있는 KPI 카드랑, 시간대별 트렌드 차트, 캠페인 효율 순위가 자동으로 만들어져요. 광고 데이터 아니어도 돼요. 판매 데이터를 넣으면 매출·주문·지역별 분석이 나오고, HR 데이터를 넣으면 인력·이탈률 분석이 나와요. 데이터를 읽고 맥락에 맞게 알아서 만들어줘요.
