...
> "campaigns 테이블을 분석해서 다음 세 가지를 알려줘. 먼저 채널별 총 지출과 총 매출. 다음으로 알오아이가 가장 높은 채널. 세 번째로 일별 총 매출 트렌드. 결과를 마크다운 리포트로 작성해서 report.md 파일로 저장해줘."
...
### git-mcp ★7,827
코드 hallucination 방지 도구. 어떤 GitHub 레포든 MCP 서버로 변환해 Claude가 정확하게 참조하게 합니다.
공식 레포: https://github.com/idosal/git-mcp

> "Claude가 내 레포 코드를 틀리게 이해하는 문제"를 해결하는 실전 도구입니다. MCP 챕터에서 "이렇게 하면 Claude가 내 코드베이스를 정확하게 인식한다"는 시연으로 임팩트가 큽니다. 설치 후 바로 써먹을 수 있는 실용적인 예시입니다.
